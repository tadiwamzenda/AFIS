...
class ReportController extends Controller
{
    ...
    public function standardReport(Request $request)
    {
        ...
        try {
            ...
            Log::error('ReportController: standard report generation failed', [
                'client_id' => $client->id,
            ...
            return back()->withErrors([
                'report' => 'The report could not be generated. This can happen for very large fleets or during a high-traffic period — please try a shorter date range or fewer groups, or contact support if the problem continues.',
            ]);
        }
    }
        ...
        public function aiReport(Request $request)
    {
        ...
        try {
            ...
            // AI failure alone degrades gracefully (report still generates
            // with all deterministic data) — kept separate from the outer
            // catch below, which guards PDF rendering/storage/DB failures.
            try {
                $aiResponse = $engine->analyze($prompt);
            } catch (\Throwable $e) {
                Log::warning('ReportController: AI analysis failed, falling back to deterministic data only', [
                    'client_id' => $client->id,
                    'error'     => $e->getMessage(),
                ]);
            ...
            Log::error('ReportController: AI report generation failed', [
                'client_id' => $client->id,
                'from'      => $from->toDateString(),
                'to'        => $to->toDateString(),
                'error'     => $e->getMessage(),
        ...
        public function clientStandardReport(Request $request)
    {
        ...
        try {
            ...
            Log::error('ReportController: client standard report generation failed', [
                'client_id' => $client->id,
                'from'      => $from->toDateString(),
                'to'        => $to->toDateString(),
                'error'     => $e->getMessage(),
            ...
            return back()->withErrors([
                'report' => 'The report could not be generated right now. Please try a shorter date range, or contact Bantu Track support if the problem continues.',
            ]);
        }
    }
        ...
        public function clientAiReport(Request $request)
    {
        ...
        try {
            ...
            try {
                ...
                Log::warning('ReportController: client AI analysis failed, falling back to deterministic data only', [
                    'client_id' => $client->id,
                    'error'     => $e->getMessage(),
                ]);
                $aiResponse = 'AI analysis unavailable: ' . $e->getMessage();
            ...
            Log::error('ReportController: client AI report generation failed', [
                'client_id' => $client->id,
                'from'      => $from->toDateString(),
                'to'        => $to->toDateString(),
                'error'     => $e->getMessage(),
