...
<body>
    <h1>Bantu Track — SIM Cards by Batch</h1>
    <p class="meta">Generated {{ now()->format('d M Y H:i') }} · {{ $groups->count() }} batches</p>

    @foreach($groups as $batch => $rows)
    <div class="group-header">{{ $batch ?: 'Unassigned' }} ({{ $rows->count() }} cards)</div>
    <table>
        <thead>
            <tr><th>ICCID</th><th>MSISDN</th><th>Provider</th><th>Type</th><th>Status</th><th>Location</th><th>Client</th></tr>
        </thead>
        <tbody>
            @foreach($rows as $sim)
            <tr>
