...
<body>
    <h1>Bantu Track — SIM Cards by Provider</h1>
    <p class="meta">Generated {{ now()->format('d M Y H:i') }} · {{ $groups->count() }} providers</p>

    @foreach($groups as $provider => $rows)
    <div class="group-header">{{ $provider }} ({{ $rows->count() }} cards)</div>
    <table>
        <thead>
            <tr><th>ICCID</th><th>MSISDN</th><th>Batch</th><th>Type</th><th>Status</th><th>Location</th><th>Client</th></tr>
        </thead>
        <tbody>
            @foreach($rows as $sim)
            <tr>
